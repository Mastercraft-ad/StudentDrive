...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Admin-only middleware to run BEFORE multer
  ...
  app.post("/api/upload-blog-image", isAuthenticated, requireAdmin, uploadBlogImage.single('file'), async (req: any, res: Response) => {
    try {
      ...
      res.status(400).json({ message: error.message || "Failed to upload blog image" });
    }
  });
  ...
  app.get("/api/blog/posts/:slug/comments", async (req: Request, res: Response) => {
    try {
      const { slug } = req.params;
      const post = await storage.getBlogPostBySlug(slug);
  ...
  app.post("/api/blog/posts/:slug/comments", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      const { slug } = req.params;
  ...
  // Blog Post Like Routes
  app.post("/api/blog/posts/:slug/like", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      const { slug } = req.params;
      const post = await storage.getBlogPostBySlug(slug);
      ...
      const hasLiked = await storage.hasUserLikedPost(post.id, req.user.id);
      if (hasLiked) {
        return res.status(400).json({ message: "You have already liked this post" });
      }

      await storage.likeBlogPost(post.id, req.user.id);
      const likeCount = await storage.getBlogPostLikeCount(post.id);
  ...
  app.delete("/api/blog/posts/:slug/like", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      const { slug } = req.params;
      const post = await storage.getBlogPostBySlug(slug);
  ...
  app.get("/api/blog/posts/:slug/like/status", async (req: any, res: Response) => {
    try {
      const { slug } = req.params;
      const post = await storage.getBlogPostBySlug(slug);
      ...
      const likeCount = await storage.getBlogPostLikeCount(post.id);
      const liked = req.user ? await storage.hasUserLikedPost(post.id, req.user.id) : false;
  ...
  app.post("/api/blog/posts/:slug/bookmark", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      const { slug } = req.params;
      const post = await storage.getBlogPostBySlug(slug);
      ...
      const alreadyBookmarked = await storage.hasUserBookmarkedPost(post.id, req.user.id);
      if (alreadyBookmarked) {
        return res.status(400).json({ message: "Post already bookmarked" });
      }

      await storage.bookmarkBlogPost(post.id, req.user.id);
      const bookmarkCount = await storage.getBlogPostBookmarkCount(post.id);
      
      res.status(201).json({ bookmarkCount, bookmarked: true });
    } catch (error: any) {
  ...
  app.delete("/api/blog/posts/:slug/bookmark", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      const { slug } = req.params;
      const post = await storage.getBlogPostBySlug(slug);
      ...
      await storage.unbookmarkBlogPost(post.id, req.user.id);
      const bookmarkCount = await storage.getBlogPostBookmarkCount(post.id);
      
      res.json({ bookmarkCount, bookmarked: false });
    } catch (error: any) {
      console.error("Error removing bookmark:", error);
      res.status(500).json({ message: "Failed to remove bookmark" });
    }
  ...
  app.get("/api/blog/posts/:slug/bookmark/status", async (req: any, res: Response) => {
    try {
      const { slug } = req.params;
      const post = await storage.getBlogPostBySlug(slug);
      ...
      const bookmarkCount = await storage.getBlogPostBookmarkCount(post.id);
      const bookmarked = req.user ? await storage.hasUserBookmarkedPost(post.id, req.user.id) : false;
      
      res.json({ bookmarkCount, bookmarked });
    } catch (error: any) {
  ...
  app.get("/api/blog/my-bookmarks", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      const posts = await storage.getUserBookmarkedPosts(req.user.id);
      res.json(posts);
    } catch (error: any) {
  ...
  app.get("/api/blog/author/:authorId/posts", async (req: Request, res: Response) => {
    try {
      const { authorId } = req.params;
      const limit = parseInt(req.query.limit as string) || 5;
      const posts = await storage.getPostsByAuthor(authorId, limit);
      res.json(posts);
    } catch (error: any) {
  ...
  app.get("/api/admin/blog/posts", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
  ...
  app.get("/api/admin/blog/posts/:id", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
  ...
  app.post("/api/admin/blog/posts", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
  ...
  app.put("/api/admin/blog/posts/:id", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
  ...
  app.patch("/api/admin/blog/posts/:id/publish", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
      ...
      const post = await storage.publishBlogPost(id, published);
      res.json(post);
    } catch (error: any) {
  ...
  app.patch("/api/admin/blog/posts/:id/featured", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
      ...
      const post = await storage.toggleFeaturedBlogPost(id, featured);
      res.json(post);
    } catch (error: any) {
  ...
  app.delete("/api/admin/blog/posts/:id", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
  ...
  app.get("/api/admin/blog/comments", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
  ...
  app.get("/api/admin/blog/comments/pending", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }

      const comments = await storage.getPendingBlogComments();
      res.json(comments);
    } catch (error: any) {
  ...
  app.patch("/api/admin/blog/comments/:id/approve", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
  ...
  app.delete("/api/admin/blog/comments/:id", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
  ...
  app.post("/api/admin/blog/categories", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
  ...
  app.put("/api/admin/blog/categories/:id", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
  ...
  app.delete("/api/admin/blog/categories/:id", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
  ...
  app.post("/api/admin/blog/tags", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
  ...
  app.put("/api/admin/blog/tags/:id", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
      ...
      const { id } = req.params;
      const validatedData = updateBlogTagSchema.parse(req.body);
  ...
  app.delete("/api/admin/blog/tags/:id", isAuthenticated, requireOnboarding, async (req: any, res: Response) => {
    try {
      if (req.user.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }
      ...
      res.json({ message: "Blog tag deleted successfully" });
    } catch (error: any) {
